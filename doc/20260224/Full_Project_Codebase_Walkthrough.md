...
## [detection.py](../../src/core/detection.py) — 362줄

> **역할**: 이미지에서 이물질(Foreign Body)과 기포(Bubble)의 **위치**를 찾는 검출 엔진.

### 클래스: [`BubbleDetectorParams`](../../src/core/detection.py#L6-L55) *(L6-55)*
...
**메서드:**
- [`get_params()`](../../src/core/detection.py#L43-L44) *(L43-44)* → `dict` 반환 (JSON 직렬화용)
- [`set_params()`](../../src/core/detection.py#L46-L55) *(L46-55)* → dict에서 파라미터 복원

---

### 클래스: [`ForeignBodyDetector`](../../src/core/detection.py#L58-L361) *(L58-361)*
실제 검출 로직을 담당하는 핵심 클래스입니다.

#### [`detect_static()`](../../src/core/detection.py#L63-L142) *(L63-142)*
...
#### [`detect_bubbles()`](../../src/core/detection.py#L151-L298) *(L151-298)*
...
#### [`_merge_contours()`](../../src/core/detection.py#L300-L354) *(L300-354)*
...
## [classification.py](../../src/core/classification.py) — 909줄
...
### 클래스: [`RuleBasedClassifier`](../../src/core/classification.py#L19-L125) *(L19-125)*
...
**주요 메서드:**
- [`classify()`](../../src/core/classification.py#L50-L54) *(L50-54)* → 단일 분류 (dict 반환)
- [`classify_batch()`](../../src/core/classification.py#L56-L125) *(L56-125)* → 일괄 분류 (NumPy 벡터 연산으로 최적화)
- [`get_params()`](../../src/core/classification.py#L37-L41) *(L37-41)* / [`set_params()`](../../src/core/classification.py#L43-L48) *(L43-48)* → 파라미터 직렬화/역직렬화

> **C++ 비유**: 이 클래스는 if-else 분기로 구현된 `Strategy Pattern`입니다.

---

### 클래스: [`DeepLearningClassifier`](../../src/core/classification.py#L128-L389) *(L128-389)*
...
#### [`load_model()`](../../src/core/classification.py#L143-L204) *(L143-204)*
...
#### [`_extract_rois_chunk()`](../../src/core/classification.py#L219-L252) *(L219-252)*
...
#### [`classify_batch()`](../../src/core/classification.py#L254-L377) *(L254-377)*
...
### 클래스: [`ParticleClassifier`](../../src/core/classification.py#L394-L422) *(L394-422)*
...
### 클래스: [`DefectImageSaver`](../../src/core/classification.py#L425-L601) *(L425-601)*
...
**주요 메서드:**
- [`save()`](../../src/core/classification.py#L508-L567) *(L508-567)* → BMP 파일 저장, 경로 반환
- [`parse_defect_filename()`](../../src/core/classification.py#L569-L587) *(L569-587)* → 파일명에서 메타데이터(좌표, 라벨) 파싱
- [`save_original()`](../../src/core/classification.py#L494-L506) *(L494-506)* → 원본 프레임 보관

---

### 독립 함수: [`_build_model(num_classes)`](../../src/core/classification.py#L660-L677) *(L660-677)*
...
### 독립 함수: [`train_classifier(data_dir, save_path, epochs, progress_callback)`](../../src/core/classification.py#L680-L908) *(L680-908)*
...
### 클래스: [`FocalLoss`](../../src/core/classification.py#L634-L657) *(L634-657)*
불균형 데이터에 효과적인 손실 함수입니다. 쉬운 샘플(Noise)의 가중치를 줄이고 어려운 샘플(Bubble, Particle)에 집중합니다.

---

## [yolo_detector.py`](../../src/core/yolo_detector.py) — 183줄

> **역할**: `ultralytics` 라이브러리의 YOLO 모델을 감싸는 래퍼 클래스.

### 클래스: [`YOLODetector`](../../src/core/yolo_detector.py#L15-L182) *(L15-182)*

| 메서드 | 설명 |
|--------|------|
| [`load_model()`](../../src/core/yolo_detector.py#L29-L63) *(L29-63)* | `.pt` 모델 로드 (ultralytics YOLO 형식) |
| [`detect()`](../../src/core/yolo_detector.py#L65-L127) *(L65-127)* | 한 번의 추론으로 위치+분류 동시 수행 |
| [`train()`](../../src/core/yolo_detector.py#L129-L182) *(L129-182)* | YOLO 모델 학습 실행 |
...
## [yolo_dataset.py`](../../src/core/yolo_dataset.py) — 413줄

> **역할**: YOLO 학습용 데이터셋 폴더 구조 생성 및 관리 유틸리티.

### 독립 함수들
- [`contour_to_yolo_bbox()`](../../src/core/yolo_dataset.py#L22-L29) *(L22-29)* → OpenCV contour → YOLO 정규화 좌표 `(cx, cy, w, h)`
- [`bbox_xyxy_to_yolo()`](../../src/core/yolo_dataset.py#L32-L39) *(L32-39)* → 절대 좌표 → 정규화 좌표
- [`yolo_to_bbox_xyxy()`](../../src/core/yolo_dataset.py#L42-L53) *(L42-53)* → 정규화 좌표 → 절대 좌표

### 클래스: [`YOLODatasetManager`](../../src/core/yolo_dataset.py#L56-L412) *(L56-412)*
...
| 메서드 | 설명 |
|--------|------|
| [`create_structure()`](../../src/core/yolo_dataset.py#L84-L88) *(L84-88)* | 폴더 구조 생성 |
| [`write_data_yaml()`](../../src/core/yolo_dataset.py#L90-L103) *(L90-103)* | `data.yaml` 작성 (학습 설정 파일) |
| [`add_image_with_labels()`](../../src/core/yolo_dataset.py#L109-L171) *(L109-171)* | 이미지+어노테이션 추가 |
| [`add_frame_with_contours()`](../../src/core/yolo_dataset.py#L173-L222) *(L173-222)* | OpenCV contour → YOLO 형식 변환 후 추가 |
| [`auto_label_image()`](../../src/core/yolo_dataset.py#L224-L283) *(L224-283)* | Threshold+RuleBase로 자동 어노테이션 생성 |
| [`split_train_val()`](../../src/core/yolo_dataset.py#L285-L305) *(L285-305)* | 랜덤 train/val 분할 |
| [`get_stats()`](../../src/core/yolo_dataset.py#L307-L335) *(L307-335)* | 이미지 수, 클래스별 통계 반환 |

---

# 📁 Part 3: 하드웨어 추상화 (`src/hardware/`)

## [camera_interface.py`](../../src/hardware/camera_interface.py) — 29줄
...
## [basler_camera.py`](../../src/hardware/basler_camera.py) — 175줄
...
### 주요 메서드 ([`BaslerCamera` 클래스](../../src/hardware/basler_camera.py#L27-L174) *(L27-174)*)

| 메서드 | 설명 |
|--------|------|
| [`open()`](../../src/hardware/basler_camera.py#L44-L88) *(L44-88)* | GigE 디바이스 탐색 → 연결 → Grabbing 시작. 실패 시 상세 오류 메시지 반환 |
| [`close()`](../../src/hardware/basler_camera.py#L90-L99) *(L90-99)* | Grabbing 중지 → 카메라 해제 |
| [`grab_frame()`](../../src/hardware/basler_camera.py#L101-L110) *(L101-110)* | `RetrieveResult()` → BGR 변환 → numpy 배열 반환 |
| [`set_exposure()`](../../src/hardware/basler_camera.py#L112-L118) *(L112-118)* | ExposureAuto=Off → ExposureTime 설정 |
| [`get_parameters_dict()`](../../src/hardware/basler_camera.py#L128-L148) *(L128-148)* | 현재 노출, 게인 등을 dict로 반환 |
| [`set_parameters_dict()`](../../src/hardware/basler_camera.py#L150-L174) *(L150-174)* | dict에서 카메라 파라미터 일괄 적용 |

> **핵심**: `_find_basler_device()` 함수가 `PREFERRED_MODEL = "acA1920-25gm"`을 우선 검색합니다.

---

## [file_camera.py`](../../src/hardware/file_camera.py) — 130줄
...
### 추가 메서드 (`CameraSource`에 없는 확장, [`FileCamera` 클래스](../../src/hardware/file_camera.py#L15-L129) *(L15-129)*)
| 메서드 | 설명 |
|--------|------|
| [`is_video()`](../../src/hardware/file_camera.py#L107-L109) *(L107-109)* | 동영상인지 정적 이미지인지 판별 |
| [`get_frame_count()`](../../src/hardware/file_camera.py#L111-L116) *(L111-116)* | 총 프레임 수 (이미지=1) |
| [`get_frame_position()`](../../src/hardware/file_camera.py#L118-L122) *(L118-122)* | 현재 재생 위치 |
| [`set_frame_position()`](../../src/hardware/file_camera.py#L124-L129) *(L124-129)* | 특정 프레임으로 시크 |
...
## [main_window.py](../../src/ui/main_window.py) — 2370줄

> **역할**: 프로그램의 **메인 윈도우(뼈대)**. 모든 UI 요소, 이벤트 처리, 타이머, 검사 파이프라인 통합이 이 파일에 있습니다.

### 클래스: [`DetectionWorker(QThread)`](../../src/ui/main_window.py#L28-L240) *(L28-240)*
...
| 메서드 | 설명 |
|--------|------|
| [`run_detection()`](../../src/ui/main_window.py#L45-L63) *(L45-63)* | 파라미터 세팅 후 스레드 시작 요청 |
| [`run()`](../../src/ui/main_window.py#L65-L80) *(L65-80)* | 스레드 본체: `_run_threshold()` 또는 `_run_yolo()` 호출 |
| [`_run_threshold()`](../../src/ui/main_window.py#L82-L208) *(L82-208)* | Threshold+Bubble 검출 → 분류 → 프로파일링 출력 |
| [`_run_yolo()`](../../src/ui/main_window.py#L210-L240) *(L210-240)* | YOLO 단일 추론 파이프라인 |

`result_ready` 시그널로 결과를 메인 스레드(UI)에 전달합니다.

---

### 클래스: [`MainWindow(QMainWindow)`](../../src/ui/main_window.py#L243-L2369) *(L243-2369)* — 핵심

#### [생성자 `__init__()`](../../src/ui/main_window.py#L244-L298) *(L244-298)* 에서 초기화하는 주요 멤버
...
#### [`init_ui()`](../../src/ui/main_window.py#L302-L752) *(L302-752)* — UI 레이아웃 구축 (약 450줄)
...
| 메서드 | 설명 |
|--------|------|
| [`connect_camera()`](../../src/ui/main_window.py#L814-L861) *(L814-861)* | Basler 카메라 연결/해제 토글 |
| [`load_file()`](../../src/ui/main_window.py#L866-L873) *(L866-873)* → [`load_image_path()`](../../src/ui/main_window.py#L875-L925) *(L875-925)* | 이미지/영상 파일 로드 |
| [`_on_realtime_view_toggled()`](../../src/ui/main_window.py#L927-L937) *(L927-937)* | 실시간 뷰 타이머 제어 |

**검사 파이프라인:**
| 메서드 | 설명 |
|--------|------|
| [`toggle_inspection()`](../../src/ui/main_window.py#L1011-L1028) *(L1011-1028)* | 검사 시작/정지 토글 |
| [`update_frame()`](../../src/ui/main_window.py#L1400-L1537) *(L1400-1537)* | 타이머에서 호출: 프레임 획득 → 검사 요청 |
| [`_on_detection_result()`](../../src/ui/main_window.py#L1034-L1222) *(L1034-1222)* | 검출 완료 후 결과 표시·저장 (시그널 슬롯) |

**ROI 및 어노테이션:**
| 메서드 | 설명 |
|--------|------|
| [`_on_roi_button_clicked()`](../../src/ui/main_window.py#L1895-L1913) *(L1895-1913)* | 검사 ROI 설정 모드 진입 |
| [`_on_annotation_roi_button_clicked()`](../../src/ui/main_window.py#L1915-L1936) *(L1915-1936)* | 어노테이션 Rect 드래그 모드 진입 |
| [`_finish_annotation_rect()`](../../src/ui/main_window.py#L1938-L2038) *(L1938-2038)* | 드래그 완료 → 라벨 선택 팝업 → 224×224 크롭 저장 |
...
| [`update_debug_panel()`](../../src/ui/main_window.py#L1773-L1806) *(L1773-1806)* | Binary/CLAHE/Diff Map 등 디버그 이미지 표시 |
| [`update_defect_view()`](../../src/ui/main_window.py#L1812-L1883) *(L1812-1883)* | 선택된 결함의 확대 이미지 + 정보 표시 |
...
## [classification_tab.py`](../../src/ui/classification_tab.py) — 765줄

> **역할**: 분류 학습 데이터 준비(이미지 등록·라벨링) 및 학습 런칭 UI.

### 클래스: [`TrainWorker(QThread)`](../../src/ui/classification_tab.py#L22-L40) *(L22-40)*
학습을 백그라운드에서 수행합니다. `progress` 시그널로 epoch별 진행률을 UI에 전달합니다.

### 유틸 함수들
| 함수 | 설명 |
|------|------|
| [`_project_root()`](../../src/ui/classification_tab.py#L47-L51) *(L47-51)* | EXE/프로젝트 기준 루트 경로 |
| [`_resolve_path()`](../../src/ui/classification_tab.py#L54-L61) *(L54-61)* | 상대 경로 → 절대 경로 변환 |
| [`_load_path_config()`](../../src/ui/classification_tab.py#L78-L88) *(L78-88)* / [`_save_path_config()`](../../src/ui/classification_tab.py#L91-L106) *(L91-106)* | `path_config.json`에서 데이터 폴더 경로 읽기/쓰기 |

### 클래스: [`ClassificationTab(QWidget)`](../../src/ui/classification_tab.py#L113-L764) *(L113-764)*
...
| 메서드 | 설명 |
|--------|------|
| [`_load_images()`](../../src/ui/classification_tab.py#L391-L398) *(L391-398)* | 파일 다이얼로그로 이미지 등록 |
| [`_load_folder_path()`](../../src/ui/classification_tab.py#L406-L448) *(L406-448)* | 폴더 내 이미지 일괄 등록 (라벨 폴더 자동 인식) |
| [`_register_images()`](../../src/ui/classification_tab.py#L450-L473) *(L450-473)* | 경로 목록 → 이미지 리스트에 추가 |
| [`_apply_label()`](../../src/ui/classification_tab.py#L562-L579) *(L562-579)* | 선택 항목에 라벨 적용 |
...
## [yolo_annotation_tab.py`](../../src/ui/yolo_annotation_tab.py) — 1029줄

> **역할**: YOLO 학습용 BBox 어노테이션 도구 및 학습 런칭 UI.

### 클래스: [`YOLOTrainWorker(QThread)`](../../src/ui/yolo_annotation_tab.py#L39-L72) *(L39-72)*
YOLO 학습을 백그라운드에서 수행합니다.

### 클래스: [`BboxCanvas(QLabel)`](../../src/ui/yolo_annotation_tab.py#L92-L315) *(L92-315)*
...
| 메서드 | 설명 |
|--------|------|
| [`_widget_to_image()`](../../src/ui/yolo_annotation_tab.py#L162-L168) *(L162-168)* | 위젯 좌표 → 이미지 좌표 (확대/축소 고려) |
| [`_image_to_widget()`](../../src/ui/yolo_annotation_tab.py#L170-L174) *(L170-174)* | 이미지 좌표 → 위젯 좌표 |

### 클래스: [`YOLOAnnotationTab(QWidget)`](../../src/ui/yolo_annotation_tab.py#L322-L1028) *(L322-1028)*
...
## [rule_params_dialog.py`](../../src/ui/rule_params_dialog.py) — 1200줄

> **역할**: Rule-based 분류 파라미터와 Bubble 검출 파라미터를 **실시간 미리보기**와 함께 조정하는 강력한 다이얼로그.

### 클래스: [`ZoomableImageView(QGraphicsView)`](../../src/ui/rule_params_dialog.py#L35-L91) *(L35-91)*
마우스 휠 확대/축소 + 드래그 팬을 지원하는 이미지 뷰어입니다. `QGraphicsScene` 위에 `QPixmap`을 놓고 `scale()`로 줌을 구현합니다.

### 클래스: [`DebugDetectionWorker(QThread)`](../../src/ui/rule_params_dialog.py#L98-L277) *(L98-277)*
파라미터를 변경할 때마다 검출을 백그라운드에서 재실행합니다. `target_view` 인자로 특정 디버그 뷰까지만 실행하여 속도를 높입니다.

### 클래스: [`RuleParamsDialog(QDialog)`](../../src/ui/rule_params_dialog.py#L284-L1199) *(L284-1199)*
...
## [basler_settings_dialog.py`](../../src/ui/basler_settings_dialog.py) — 161줄

> **역할**: Basler 카메라의 노출(Exposure)과 게인(Gain)을 조절하는 간단한 설정 다이얼로그.

### 클래스: [`BaslerSettingsDialog(QDialog)`](../../src/ui/basler_settings_dialog.py#L22-L160) *(L22-160)*
...
## [verify_algorithm.py](../../verify_algorithm.py)
...
> [!TIP]
> 이 프로젝트의 핵심 설계 철학은 **"관심사의 분리"**입니다. 검출(detection.py)은 "어디에", 분류(classification.py)는 "무엇인지"만 담당하고, UI(main_window.py)가 이 둘을 조합합니다. C++에서의 MVC 패턴과 매우 유사합니다.
